...
const EventoryResultItem = ({ event }) => {
    ...
    const likeButtonOnclickHandler = async (e) => {
        ...
        const { payload: { message } = {} } = await dispatch(
            likeEventoryThunk({ eventId: event._id, action: action })
        );
        console.log(message);
                    ...
                    <div>
                        <div className="float-end">
                            <button
                                ...
                                onClick={likeButtonOnclickHandler}>
                                {interested ? (
                                    <>
                                        <div className="d-inline me-2">Liked</div>
                                        <i className="d-inline bi bi-heart-fill wd-like"></i>
                                    </>
                                ) : (
                                    <>
                                        <div className="d-inline me-2">Like</div>
                        ...
                        <div className="text-muted">{intCount} interested</div>
                    </div>
                </div>
            </div>
        </div>
